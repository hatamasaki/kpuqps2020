...
#ビジュアライズしようぜ！
x1 <- coefplot(result1, intercept = FALSE)
x1
#ggplotだと簡単に保存できる！
ggsave("reg1.png")
...
reg2 <- coefplot(result2, intercept = FALSE,lwdOuter = 1,
                  newNames = c(edu = "教育程度",
                               ...
                               citysize3 = "10枚以上都市", 
                               citysize4 = "政令市特別区", 
                               pid_nindependent = "政党指示なし", 
                               pid_nopposit_party = "野党支持", 
                               ideology = "政治的イデオロギー"))
reg2
ggsave("reg2.png")

#plottong（色々いじる）
hatax1 <- coefplot(result2, intercept = FALSE, lwdOuter = 1,
                xlab = "係数の推定値", 
                ylab = "自民党感情温度に与える効果", 
                title = "",
                newNames = c(edu = "教育程度",
                               ...
                               pid_nopposit_party = "野党支持", 
                               ideology = "政治的イデオロギー")) 
#describeと保存（working directoryに保存）
hatax1
ggsave("reg2x.png")
...
#SNSの利用と年齢の効果
sns <- q5_1_2
sns[q5_1_2==1 | q5_1_2==2 |q5_1_2==3] <- "use"
sns[q5_1_2==4] <- "nouse"
sns[q5_1_2==5] <- NA
sns1 <- as.factor(sns)

r4 <- lm(LDP ~ age + gender + edu + income + citysize + ideology*sns1 , data = data2020)                   
summary(r4)




jtools::interact_plot(r4, pred = sns1, modx = ideology,
                      plot.point = FALSE, 
                      legend.main = "イデオロギー") +
  theme_bw(base_family = "HiraKakuProN-W3") + 
  labs(x = "SNS利用", y = "自民党感情温度")  +
  ylim(0, 100)


r5 <- lm(LDP ~ sns1 + gender + edu + income + citysize + ideology*age , data = data2020)                   
summary(r5)

jtools::interact_plot(r5, pred = age, modx = ideology,
                      plot.point = FALSE, 
                      legend.main = "イデオロギー") +
  theme_bw(base_family = "HiraKakuProN-W3") + 
  labs(x = "年齢", y = "自民党感情温度")  +
...
p1 <- ggplot(data2020, aes(x = gender, y = LDP)) + scale_y_continuous(breaks=seq(50,60,by=1),limits=c(50,60)) +
  stat_summary(fun.data = "mean_se")+xlab('性別')
print(p1+ ggtitle('自民党感情温度'))
